...
static void prvSetupHardware() {
	...
	LPC_SCT2->CTRL_U |= (72 - 1) << 5; 			// Set prescaler. Sysclock / 72 = 1MHz
	LPC_SCT2->EVENT[0].STATE = 0xFFFFFFFF; 		// States in which Event N will occur
	LPC_SCT2->EVENT[0].CTRL = 0 | 1 << 12; 		// Event N will occur when Match N occurs
	LPC_SCT2->EVEN = 1 << 0;					// Event N will trigger SCT_EVT_N interrupt
	LPC_SCT2->OUT[0].SET = 1 << 0; 				// Event N will set SCT2_OUTN pins to high
	LPC_SCT2->OUT[0].CLR = 1 << 0;
	LPC_SCT2->RES = 3 << 0;
...
#if WITH_RIT == 1
	Plotter::vInit([](size_t pulses_per_second) {
		Chip_RIT_SetCounter(LPC_RITIMER, 0);
		Chip_RIT_Disable(LPC_RITIMER);
		Chip_RIT_SetCompareValue(LPC_RITIMER, SystemCoreClock / pulses_per_second);
...
#elif WITH_SCT == 1
	Plotter::vInit([](size_t pulses_per_second) {
		LPC_SCT2->MATCHREL[0].U = 1'000'000 / pulses_per_second - 1;	// Number of cycles required to trigger Match N
